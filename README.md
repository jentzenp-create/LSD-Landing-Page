...
- **Required setup:**
  - `ANTHROPIC_API_KEY` must be set as a (non-`VITE_`) environment variable in your Vercel project — it's read server-side by `api/onboarding-chat.ts` (using Claude Haiku 4.5) and is never exposed to the browser.
